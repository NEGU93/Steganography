function [finalString, hitPercentage] = decoderStegFinal(imgPath, quality, spreadSpectrum,DCTSize, Repetition)
...
if spreadSpectrum
 ...
 % usar la tabla según la calidad requerida
    ...
    %----------Abrir foto, pasar a YCbCr--------------
    [myOriginalImg, colorMap] = imread(imgPath);
    %figure();
    %image(myOriginalImg);
    ...
    for myColorComponent=1:oImgComponents
        ...
        for i=0:dctRowBlocks-1
           for j=0:dctColBlocks-1
            ...
            %---DECODIFICACIÓN, por LSB en todos los coeficientes DCT salvo DC y en bloques de los bordes. Salvo spreadSpectrum----------
...
for largo=1:40
    ...
    goodString = zeros(1, largo);
    stego = zeros(1, ceil(numberOfCharacters/largo));
    Frequency = zeros(1, ceil(numberOfCharacters/largo));
    for i=1:largo
        j = i;
        n = 1;
        while n <= ceil(numberOfCharacters/largo)
     ...
     %disp(cast(goodString, 'char'));
     Hit = sum(Frequency) / ( largo * ceil(numberOfCharacters/largo));
     %disp(Hit * 100);
     if(Hit>bestHit)
        finalString=goodString; 
        bestHit=Hit;
     end
    ...
    finalString=cast(finalString, 'char');
    hitPercentage=bestHit*100;
