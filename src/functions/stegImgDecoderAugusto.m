function [myDecodedString] = stegImgDecoderAugusto(imgPath, quality)
%stegImgDecoder
...
%       2 - (Optional) la calidad de la imagen. Sino usa una por default
...
%Leer el mensaje
   ...
   %---Ajuste para que entren todos los bloques de 8x8
    ...
    %---Conversión a YCbCr
    YCbCrMap=rgb2ycbcr(myAdjustedImg);
    ...
    for myColorComponent=1:oImgComponents
        ...
        for i=0:dctRowBlocks-1
           for j=0:dctColBlocks-1
            ...
            if not((i==(dctRowBlocks-1))||(j==(dctColBlocks-1)))
            ...
            %DCTCoefficients1DArray=cast(DCTCoefficients1DArray,'double');
           end
        end
    end

    %Debo pasar los bits a bytes de caracteres de string
    numberOfCharacters=ceil((globalBitStringIndex-1)/8);
    for i=0:numberOfCharacters-1
        myDecodedString(i+1)=bi2de(fliplr(DecodedStegStringBits(i*8+1:i*8+8)));
    end
