function [myFinalEncodedImage] = stegImgEncoderAugusto(stegString, imgPath, quality)
%stegImgEncoder
...
% esteganografiado en ella.
   ...
   %---Mensaje a esteganografiar---
    %stegString='Hola Mundo';
    stegStringBits = zeros(1, length(stegString)*8);
    ...
    for n=0:length(asciiString) - 1
       ...
       for bit=8:-1:1
          ...
          stegStringBits(n*8+(9-bit)) = myBit;
        end
    end
 ...
 % usar la tabla según la calidad requerida
    ...
    LuminanceOffset=ones(oImgRows,oImgCols);
    ...
    for myColorComponent = 1:oImgComponents
        ...
        for i=0:dctRowBlocks-1
           for j=0:dctColBlocks-1
            ...
            if not((i == dctRowBlocks - 1)||(j == dctColBlocks - 1)) % mientras que no sea en los utlimos (son peligrosos)
                for iterator = 2:BLOCK_SIZE % iterar pero no en la continua
                    if stegStringBits(stegStringIndex) % si tengo que poner un 1
                        ...
                        if mod(floor(DCTCoefficients1DArray(iterator)),2)
                            ...
                            DCTCoefficients1DArray(iterator)=DCTCoefficients1DArray(iterator)+1;
                        end
    ...
    myEncodedImage = myEncodedImage(1:oImgRows, 1:oImgCols, 1:oImgComponents);
     myEncodedImage(:,:,1)=myEncodedImage(:,:,1)+LuminanceOffset;    %Fix OFFSET
